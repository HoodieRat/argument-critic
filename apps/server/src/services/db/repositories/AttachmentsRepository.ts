...
export class AttachmentsRepository {
  ...
  public createAttachment(input: {
    ...
    this.database
      .prepare(
        `INSERT INTO attachments (id, session_id, type, path, mime_type, width, height, content_hash, created_at)
         VALUES (@id, @sessionId, @type, @path, @mimeType, @width, @height, @contentHash, @createdAt)`
      )
      .run({
        ...input,
